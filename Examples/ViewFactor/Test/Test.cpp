...
int main() {
  // getRadius Test
  BoschProcessDataType<double> data;
  data.numCycles = 100;
  data.startWidth = 2 * 0.4;
  data.bottomWidth = 2 * 0.4 * 0.139484;
  data.taperStart = -24.4034;
  data.depthPerCycle = -0.369748;
  data.isoRate = data.depthPerCycle * 0.6;
  data.numTaperCycles = 34;
  data.taperRatio = 0.0267678;
  data.trenchBottom = -30.1916;
  data.gridDelta = 0.05;
  auto boschDist = lsSmartPointer<BoschDistribution<double, 2>>::New(data);

  for (double z = data.taperStart; z > data.trenchBottom; z -= data.gridDelta) {
    std::cout << z << ": " << std::endl;

    std::cout << boschDist->getRadius(z) << std::endl;
  }
  // std::cout << boschDist->getRadius(-24.0) << std::endl;
  // // bisection test
  // auto func = [](double x){ return 2 - x*x; };
  // lsBisect bisection(func, 0, 12);
  // bisection.apply();

  // std::cout << bisection.getRoot() << std::endl;
