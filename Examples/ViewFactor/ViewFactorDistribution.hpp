...
public:
  ...
  T viewfactor (double point[D], double normal[D], double start[D], double end[D]) const {
  
    double tanStart[D] = {start[0]-point[0], start[1]-point[1]};  // set point at 0,0
    double tanEnd[D]   = {end[0]-point[0], end[1]-point[1]};      // set point at 0,0

    if ((tanEnd[0] == 0) && (tanEnd[1] == 0)) tanEnd[0] = -1.;

    double adjS = dot(normal, tanStart);          //since cosX = dot(v1,v2)/mag(v1)*mag(v2) = adj/hyp
    ...
    if (tanStart[0]*tanEnd[0] < 0.) {
      double cosN = std::abs(normal[0])/std::sqrt(normal[0]*normal[0]+normal[1]*normal[1]);
...
//    return curT*data.topRate;
  ...
  T getThickness(const std::array<hrleCoordType, 3> &initial, 
    ...
    // Forward euler
    if (auto it=map.find(initial); it != map.end()){
      thickness = it->second;
    } else {
      while(curT<data.processTime){
        thickness += delT*drdt(curT, thickness, getTopThickness(curT), initial, normal);
        curT  += delT;
      }
  ...
  ViewFactorDistribution(ViewFactorProcessDataType<T> &processData, mapType &map)
      : data(processData), closingTime(data.trenchDiameter/data.topRate), map(map) {
      ...
      T vf = 0.5;// + data.taperAngle[1];
      T rate = vf*data.topRate;
      topThickness[0] = 0.;
      T normTop[D] = {1., 0};
      T d = data.trenchDiameter;
      T radX = 0;
      // i = curT/timeStep -> timeStep = curT/topThickness.size() -> curT = i*timeStep
...
//    T topTh = data.topRate*data.processTime/2.;
    ...
    if (std::abs(radius) <= data.gridDelta) {
      ...
      if (v[1] > ellipseMid) {  // Top ellipse
        ...
        T y = v[1] - ellipseMid;
        if (std::abs(x) <= topTh)
          distY = std::abs(y) - ellipseMid*std::sqrt((1.-x*x/(topTh*topTh)));
        if (std::abs(y) <= ellipseMid)
          distX = std::abs(x) - topTh*std::sqrt((1.-y*y/(ellipseMid*ellipseMid)));
        distance = (std::abs(distY)<std::abs(distX))?distY:distX;
      } else if (v[1] >= 0.) {  // Tangent between top and bottom ellipse
        distance = std::abs(v[0]) - (radius + v[1]*(topTh - radius)/ellipseMid);
      } else {  // Bottom ellipse
        T x = v[0];
        T y = v[1];
        if (std::abs(x) <= radius) 
          distY = std::abs(y) - ellipseMid*std::sqrt((1.-x*x/(radius*radius/4)));
        if (std::abs(y) <= ellipseMid)
          distX = std::abs(x) - radius*std::sqrt((1.-y*y/(ellipseMid*ellipseMid)));
        distance = (std::abs(distY)<std::abs(distX))?distY:distX;
      }
    } else { // Sidewall
      for (unsigned i = 0; i < D; ++i) {
